...
## 目录
- [Scaling-Laws](#Scaling Laws for LLMs)
...
<details><summary>subword（BPE 和 wordpiece方法）和传统word-base tokenization优势：</summary>

- 传统tokenization优缺点：
  - 表示方法无法很好的处理未知或罕见的词汇（OOV 问题）；
...
![km-scaling-law](../assets/optimization-setting.png)
综述论文建议在构建LLM时采用如下结构：
...
<font color="FF0000">【以上attention，都是在原始Full attention做近似处理，在模型性能损失 和 attention计算效率提升之间做trade-off，对attention模块就行调整】</font>
...
<font color="FF0000">【Flash attention 和 Paged attention都是从如何更高效的利用过GPU资源，来提升推理性能，前者基于GPU 不同memory level的IO计算能力，后者基于提升cache利用率】</font>


## Pretrain-task【预训练目标】：

|架构|Attention|训练目标|特点|适合任务|缺点|
|-|--|--|--|--|--|
|decoder-only|casual attention|language model task|in-context learning，few-shot能力|长文本生成任务，尤其是文本生成、问答生成、对话生成、摘要生成、生成式翻译等|因为是对未来预测，其训练的难度大|
|prefix LM|prefix casual attention| auto-regression blank infilling|训练目标包括short-span的NLU目标，以及documents level和setence level两个长spans的文本生成目标；通过调整span长度和数量，实现分别训练NLU、LM、seq-seq多目标|NLU任务，（有条件和无条件的）文本生成任务中表现出色|本质为多目标训练，训练的难度以及样本构造的难度都很大；在相同训练tokens数量下，效果不如decoder-only|
|encoder-decoder|encoder部分fully attention，decoder部分casual attention|DAE（denoising auto-encoding task）|统一了NLU和NLG任务（text-to-text任务），以自回归的方式训练，以恢复被替换的部分|适合NLU和“有条件”的文本生成任务，例如文本总结，respose生成等|但需要针对性的构造任务数据的输入形式和前缀，且参数量量翻倍，训练成本较高；在NLU和文本生成任务也不如prefix LM|
|encoder-only|fully attention|MLM| 充分学习到前后文本内在的联系，更加适合NLU相关的任务|NLU问题：语义理解、情感分析、意图理解、实体抽取、关系抽取等|在同等参数和计算消耗情况下，性能不如prefix LM|
| |  | | | | |
|MoE|以Switch Transformer 为例，在进行Attention计算之后，通过Router模块， 构建FNN多专家网络，以达到稀疏多专家目的；| | | | |
|Emergent-Architectures|这些新架构的关键优点有两个：1）首先，这些模型可以像RNN一样递归生成输出，意味着它们在解码过程中只需要参考单个先前状态。这样做可以提高解码过程的效率，因为它消除了传统Transformer中需要重新访问所有先前状态的需求； 2）其次，这些模型具有像Transformer一样并行编码整个句子的能力，可以利用GPU的并行性，结合诸如Parallel Scan 、FFT 和Chunkwise Recurren等技术。这些技术使得具有这些新架构的模型能够以高度并行和高效的方式进行训练；| | | | |
...
which is designed for pre-training models with the prefix decoder architecture. The tokens within a randomly selected prefix would not be used in computing the loss of prefix language modeling. With the same amount of tokens seen during pretraining, prefix language modeling performs slightly worse than language modeling, since fewer tokens in the sequence are involved for model pre-training；
  - 【prefix LM采用auto-regressive blank infilling目标，包括short-span的NLU目标，以及documents level和setence level两个长spans的文本生成目标；本质为多目标训练，训练的难度以及样本构造的难度都很大】
  - 【在相同训练tokens情况下，prefix LM效果没有decoder-only好，原因在于用于目标loss训练的tokens较少】
...
<details><summary>在指令构造上遵循如下原则：</summary>
...
> 从困难度上看，Honesty>Harmless>Helpful


1. 【pretrain，以LM为目标，进行文本生成训练，就像在海纳百川般的接受各种知识，但LLM实际上可能并不是很理解这些知识；或者没有通，只是貌似记住了知识，可以做一些机器式交互；】
———— **海纳百川各种学**；

2. 【Instruction Tuning，以sequence-to-sequence为目标，以人类自然语言的指令实例，让模型理解人类自然语言，以获取强大泛化能力；这种激发出来的能力，可以使得模型在其他一些未知任务具备了处理能力】【有点类似举一反三】
———— **理解人类自然语言，具备完成人类自然语言形式的任务，包括对话**；

3. 【Alignment Tuning，对齐人类价值观或者偏好，考虑不同的标准（例如helpful、honesty、harmless），以使LLM返回结果与人类期望一致，从目标上看，Alignment标准看上去更难，不论是数据的构建，还是训练微调方式】
———— **进一步实现人类价值偏好的对齐**；
...
|训练方式| 流程                                                                                                                                                                                                                                                     | 问题点                                                                                                                                         | 一些技巧                                                                                                                                                                                                                                                                                                                                                                      |
|-|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|RLHF| 流程复杂：<li> 1）SFT：基于人工标注的优质指令数据，在LLM上进行有监督微调得到SFT，使得LLM具备理解人类自然语言能力；<li> 2）RM：在SFT基础上，对同一指令输入下，不同的输出结果进行人工偏好rank标注，以此训练一个RM模型，来评估一组<指令、输入、输出>得分，得分越高，说明越符合人类偏好；<li> 3）RL：以RM模型输出结果作为reward，采用RL（常见PPO算法）来对LLM进行微调，使之返回符合人类价值偏好（helpful、harmless、honesty）； | <li>1）SFT阶段：以高质量指令数据为主，人工标注难度和成本较大；<li> 2）RM阶段：采用较大RM模型在判断LLM输出质量方面表现更好，但对应带来训练不稳定，和过拟合风险；<li> 3）RL阶段：训练成本高，RL训练存在不稳定，且PPO算法相当复杂，对超参数敏感等问题； | <li> 1）SFT阶段：参考 Instruction tuning；<li> 2）RM阶段：1. 对齐SFT和RM基模型，减少两者之间信息不匹配度；2. 训练多个不同RM模型，不同RM关注不同标准（helpful、harmless、honesty），计算reward时采用组合策略方式，在后续调试中更佳灵活；<li> 3）RL阶段：1. 在SFT模型上，通过最优输出数据进行微调，以达到一个较好的初始LLM，同时结合多个RM模型，产生多个版本RLHF，可清晰知道后续迭代方向；2. beam search decoding算法，同是生成多个具有多样性的输出结果；3. 少训练时服务器计算和存储成本，可将RM模型进行部署，采用API调用方式；                                         |
|SFT|在高质量对齐数据集上进行监督学习，直接让LLM学习人类偏好；1）对齐数据集构造； 2）微调算法，或者微调损失函数的构造；除原始指令微调的损失函数，还增加辅助学习目标来增强对齐性能，例如对响应进行排序，或者做对比学习比较<instruction，input，respose>| <li> 1）数据集：完全依赖监督学习标注“高质量数据”，但不同标注者存在个体偏好、风格等差异，导致数据间存在差异性；<li> 2）目标函数：<li> 3）建模算法：| <li> 1）数据集：1. 基于已有RM模型，来对生成的respose进行打分，从而选择高质量的respose作为后续微调对齐训练数据集；2. LM文本生成能力，通过构造相关行为原则（实际为自然语言指令），让LLM对自己的respose进行评估，如此循环，最终得到align数据；《降低RM构造成本，也避免RM模型和LLM模型不匹配问题》；<li> 2）目标函数：1. Loss：监督学习，sequence-to-sequence cross entropy loss；2. 习目标：增加排序相关的损失函数，来对不同质量的respose做偏好建模；<li> 3）建模算法：1.对比学习：提高正确指令-respose对的概率，同时降低不正确的指令-respose对的概率，可以使模型学习到指令和respose之间的正确关联； |
